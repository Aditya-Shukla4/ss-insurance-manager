...
import { type Client, type Policy, type UserProfile } from "@/lib/types";
                ...
                {renewals7Days.map(r => (
                  <li key={r.id} className="text-sm border-b pb-2 last:border-b-0 hover:bg-orange-50/50 p-1 rounded">
                    ...
                    <div className="flex justify-between items-center text-gray-500 mt-1">
                      <span>Due: <span className="font-medium text-orange-700">{new Date(r.due_date).toLocaleDateString()}</span></span>
                      <span>Premium: <span className="font-medium">₹{r.premium?.toLocaleString('en-IN') ?? 'N/A'}</span></span>
                    </div>
                  </li>
                ))}
              </ul>
              ...
              <ul className="space-y-3 max-h-60 overflow-y-auto pr-2">
                {renewals30Days.map(r => (
                  <li key={r.id} className="text-sm border-b pb-2 last:border-b-0 hover:bg-blue-50/50 p-1 rounded">
                    ...
                    <p className="text-gray-600">{r.plan_name}</p>
                    <div className="flex justify-between items-center text-gray-500 mt-1">
                      <span>Due: <span className="font-medium">{new Date(r.due_date).toLocaleDateString()}</span></span>
                      <span>Premium: <span className="font-medium">₹{r.premium?.toLocaleString('en-IN') ?? 'N/A'}</span></span>
                    </div>
                  </li>
                ))}
